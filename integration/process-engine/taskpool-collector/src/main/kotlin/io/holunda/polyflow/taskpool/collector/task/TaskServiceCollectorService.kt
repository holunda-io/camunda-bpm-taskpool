            ...
            commandExecutor.execute { innerContext ->
                innerContext.registerCommandContextListener(object : CommandContextListener {
                    override fun onCommandContextClose(commandContext: CommandContext) {
                        ...
                        (commandContext.sessions[DbEntityManager::class.java] as? DbEntityManager)?.dbEntityCache?.apply {
                            cachedEntities.forEach {
                                remove(
                                    it
                                )
                            }
                ...
                // create commands
                val commands = engineTasks.filterIsInstance<TaskEntity>()
                    .map { task -> task.asCreateCommand(camundaTaskpoolCollectorProperties.applicationName) }
