...
internal class SimpleTaskPoolServicePerformanceTest {
  private val service = SimpleTaskPoolService(mock(QueryUpdateEmitter::class.java))
  ...
  fun `can handle lots of data`() {
    Flux.range(1, 10_000)
      ...
      .doOnNext{service.on(DataEntryCreatedEvent("type", "id-$it", "", "", ""))}
    ...
    Flux.range(1, 10_000)
      .publishOn(Schedulers.parallel())
      .doOnNext{service.on(TaskCreatedEngineEvent("task-$it", ProcessReference("", "", "", "", "", ""), ""))}
      .sample(Duration.ofSeconds(1))
      .timed()
      .doOnNext{ println("Added ${it.get()} Tasks in ${it.elapsedSinceSubscription()}")}
      .blockLast()

    Assertions.assertThat(service.query(TasksForApplicationQuery("")).totalElementCount).isEqualTo(10_000)
  }
}
