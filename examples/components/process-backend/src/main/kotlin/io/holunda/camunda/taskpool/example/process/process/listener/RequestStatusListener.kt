...
import io.holunda.camunda.taskpool.example.process.process.RequestApprovalProcess.Elements.AMEND_REQUEST
import io.holunda.camunda.taskpool.example.process.process.RequestApprovalProcess.Elements.APPROVE_REQUEST
import io.holunda.camunda.taskpool.example.process.process.RequestApprovalProcess.Elements.AUDIT_SUBMITTED
  ...
  fun notifyState(execution: DelegateExecution) {
    ...
    when (execution.currentActivityId) {
      AUDIT_SUBMITTED -> {
        ...
        notifyChange(request = request, state = IN_PROGRESS.of("Submitted"), log = "New approval request submitted.", revision = revision, username = originator)
      }
      else -> Unit
    }
  }
    ...
    when (task.taskDefinitionKey) {
      APPROVE_REQUEST -> {
        ...
        when (approvalDecision) {
          APPROVE -> {
            notifyChange(request = request, state = COMPLETED.of("Approved"), log = "Request successfully approved.", revision = updateAndStoreNewRevision(task), username = task.assignee)
          }
          REJECT -> {
            notifyChange(request = request, state = COMPLETED.of("Rejected"), log = "Request rejected by approver ($user).", revision = updateAndStoreNewRevision(task), username = task.assignee)
          }
          RETURN -> {
            notifyChange(request = request, state = IN_PROGRESS.of("Returned"), log = "Request returned to originator.", revision = updateAndStoreNewRevision(task), username = task.assignee)
          }
          else -> Unit
        }
      }
      AMEND_REQUEST -> {
        ...
        when (amendAction) {
          RESUBMIT -> {
            notifyChange(request = request, state = IN_PROGRESS.of("Resubmitted"), log = "Request resubmitted for approval.", revision = updateAndStoreNewRevision(task), username = task.assignee)
          }
          CANCEL -> {
            notifyChange(request = request, state = IN_PROGRESS.of("Cancelled"), log = "Request cancelled by originator.", revision = updateAndStoreNewRevision(task), username = task.assignee)
          }
          else -> Unit
        }
